...
processor, model = load(device="cuda" if torch.cuda.is_available() else None)
...
def extract(photo: str | None, use_preprocess: bool = True) -> dict:
    if not photo:
...
with gr.Blocks(title="Lectura") as demo:
    ...
    use_preprocess = gr.Checkbox(value=True, label="Correct perspective and lighting")
